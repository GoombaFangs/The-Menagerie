#include "loading_screen.h"

void title()
{
    double durtion = 0.05;
    hold_seconds(durtion);
    reset_console();
    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");
    hold_seconds(durtion);
    reset_console();
    printf("                         __                 \n");
    printf("                         /  | ___ _ __   __ _\n");
    printf("                        |\\/| |/ _ \\ '_ \\ / _` |\n");
    printf("                         | |  __/ | | | (_| \n");
    printf("                          |_|\\___|_| |_|\\__,_|\n");
    printf("                                            \n");
    hold_seconds(durtion);
    reset_console();
    printf("                     __  __                       \n");
    printf("                    |  \\/  | ___ _ __   __ _  __ _\n");
    printf("                   \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |\n");
    printf("                    | |  | |  __/ | | | (_| | (_| \n");
    printf("                   | |_|  |_|\\___|_| |_|\\__,_|\\__, |\n");
    printf("                                             |___/\n");
    hold_seconds(durtion);
    reset_console();
    printf("                     __  __                             \n");
    printf("               ___  |  \\/  | ___ _ __   __ _  __ _  ___ \n");
    printf("            \\ / _ \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |/ _ \\ \n");
    printf("             |  __/ | |  | |  __/ | | | (_| | (_| |  __/\n");
    printf("            _|\\___| |_|  |_|\\___|_| |_|\\__,_|\\__, |\\___|_\n");
    printf("                                             |___/      \n");
    hold_seconds(durtion);
    reset_console();
    printf("      _ _            __  __                                  _\n");
    printf("      _| |__   ___  |  \\/  | ___ _ __   __ _  __ _  ___ _ __(_\n");
    printf("       | '_ \\ / _ \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |/ _ \\ '__| |\n");
    printf("       | | | |  __/ | |  | |  __/ | | | (_| | (_| |  __/ |  | \n");
    printf("       |_| |_|\\___| |_|  |_|\\___|_| |_|\\__,_|\\__, |\\___|_|  |_|\n");
    printf("                                             |___/            \n");
    hold_seconds(durtion);
    reset_console();
    printf("  _____ _            __  __                                  _      \n");
    printf(" |_   _| |__   ___  |  \\/  | ___ _ __   __ _  __ _  ___ _ __(_) ___ \n");
    printf("   | | | '_ \\ / _ \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |/ _ \\ '__| |/ _ \\\n");
    printf("   | | | | | |  __/ | |  | |  __/ | | | (_| | (_| |  __/ |  | |  __/\n");
    printf("   |_| |_| |_|\\___| |_|  |_|\\___|_| |_|\\__,_|\\__, |\\___|_|  |_|\\___|\n");
    printf("                                             |___/                  \n");
    hold_seconds(2);/////
    reset_console();
    printf("      _ _            __  __                                  _\n");
    printf("      _| |__   ___  |  \\/  | ___ _ __   __ _  __ _  ___ _ __(_\n");
    printf("       | '_ \\ / _ \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |/ _ \\ '__| |\n");
    printf("       | | | |  __/ | |  | |  __/ | | | (_| | (_| |  __/ |  | \n");
    printf("       |_| |_|\\___| |_|  |_|\\___|_| |_|\\__,_|\\__, |\\___|_|  |_|\n");
    printf("                                             |___/            \n");
    hold_seconds(durtion);
    reset_console();
    printf("      _ _            __  __                                  _\n");
    printf("      _| |__   ___  |  \\/  | ___ _ __   __ _  __ _  ___ _ __(_\n");
    printf("       | '_ \\ / _ \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |/ _ \\ '__| |\n");
    printf("       | | | |  __/ | |  | |  __/ | | | (_| | (_| |  __/ |  | \n");
    printf("       |_| |_|\\___| |_|  |_|\\___|_| |_|\\__,_|\\__, |\\___|_|  |_|\n");
    printf("                                             |___/            \n");
    hold_seconds(durtion);
    reset_console();
    printf("                     __  __                             \n");
    printf("               ___  |  \\/  | ___ _ __   __ _  __ _  ___ \n");
    printf("            \\ / _ \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |/ _ \\ \n");
    printf("             |  __/ | |  | |  __/ | | | (_| | (_| |  __/\n");
    printf("            _|\\___| |_|  |_|\\___|_| |_|\\__,_|\\__, |\\___|_\n");
    printf("                                             |___/      \n");
    hold_seconds(durtion);
    reset_console();
    printf("                     __  __                       \n");
    printf("                    |  \\/  | ___ _ __   __ _  __ _\n");
    printf("                   \\ | |\\/| |/ _ \\ '_ \\ / _` |/ _` |\n");
    printf("                    | |  | |  __/ | | | (_| | (_| \n");
    printf("                   | |_|  |_|\\___|_| |_|\\__,_|\\__, |\n");
    printf("                                             |___/\n");
    hold_seconds(durtion);
    reset_console();
    printf("                         __                 \n");
    printf("                         /  | ___ _ __   __ _\n");
    printf("                        |\\/| |/ _ \\ '_ \\ / _` |\n");
    printf("                         | |  __/ | | | (_| \n");
    printf("                          |_|\\___|_| |_|\\__,_|\n");
    printf("                                            \n");
    hold_seconds(durtion);
    reset_console();
    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");
    reset_console();
	hold_seconds(0.5);
}

void print_ship()
{
    printf("               __________________                                \n");
    printf("              /==\\_______________\\                                \n");
    printf("             |                     >_________________________________\n");
    printf("    ---------|    ___     ___     ___     ___     ___     ___        \\\n");
    printf("    \\        |   |___|   |___|   |___|   |___|   |___|   |___|         >____________\n");
    printf("    /         \\__/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\_________/            \\\n");
    printf("   |________     |===|   |===|   |===|   |===|   |===|   |===|                       |\n");
    printf("           \\_____________________                                   __________________/\n");
    printf("                     /          /                                  /          \n");
    printf("                    /__________/          _______________________/          \n");
    printf("                              /___________/          \n");

}

void print_ship_leave_planet(char* planet_terrain, int got_alien)
{
    double durtion = 0.2;
    hold_seconds(durtion);
    reset_console();
    printf("               __________________                                \n");
    printf("              /==\\_______________\\                                \n");
    printf("             |                     >_________________________________\n");
    printf("    ---------|    ___     ___     ___     ___     ___     ___        \\\n");
    printf("    \\        |   |___|   |___|   |___|   |___|   |___|   |___|         >____________\n");
    printf("    /         \\__/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\_________/            \\\n");
    printf("   |________     |===|   |===|   |===|   |===|   |===|   |===|                       |\n");
    printf("           \\_____________________                                   __________________/\n");
    printf("                     /          /                                  /          \n");
    printf("                    /__________/          _______________________/          \n");
    printf("                              /___________/          \n");
    print_planet(planet_terrain);
    if(got_alien != -1) printf("\n\n\n YOU GOT A NEW  ALIEN\n");
    hold_seconds(durtion);
    reset_console();
    printf("                           __________________                        \n");
    printf("                          /==\\_______________\\                        \n");
    printf("                         |                     >_________________________\n");
    printf("                ---------|    ___     ___     ___     ___     ___     ___  \n");
    printf("               \\        |   |___|   |___|   |___|   |___|   |___|   |___|         >____\n");
    printf("                /         \\__/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\_________/    \\\n");
    printf("               |________     |===|   |===|   |===|   |===|   |===|   |===|                \n");
    printf("                       \\_____________________                                   ___________\n");
    printf("                                 /          /                                  /  \n");
    printf("                                /__________/          _______________________/  \n");
    printf("                                          /___________/  \n");
    print_planet(planet_terrain);
    if (got_alien != -1) printf("\n\n\n YOU GOT A NEW  ALIEN\n");
    hold_seconds(durtion);
    reset_console();
    printf("                                                 __________________              \n");
    printf("                                                /==\\_______________\\              \n");
    printf("                                               |                     >_______________\n");
    printf("                                      ---------|    ___     ___     ___     ___     ___\n");
    printf("                                     \\        |   |___|   |___|   |___|   |___|   |___|   |___|    \n");
    printf("                                 0 00 /         \\__/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\______\n");
    printf("                                     |________     |===|   |===|   |===|   |===|   |===|   |===|      \n");
    printf("                                             \\_____________________                                   _\n");
    printf("                                                       /          /                           \n");
    printf("                                                      /__________/          ________________\n");
    printf("                                                                /____\n");
    print_planet(planet_terrain);
    if (got_alien != -1) printf("\n\n\n YOU GOT A NEW  ALIEN\n");
    hold_seconds(durtion);
    reset_console();
    printf("                                                                    __________________  \n");
    printf("                                                                   /==\\_______________\\  \n");
    printf("                                                                  |                     >___\n");
    printf("                                                         ---------|    ___     ___     ___    \n");
    printf("                                                        \\        |   |___|   |___|   |___|   |___|   |___|\n");
    printf("                                        0 0 0 0 0 0 0000 /         \\__/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\___\n");
    printf("                                                        |________     |===|   |===|   |===|   |===|   |===| \n");
    printf("                                                                \\_____________________                        \n");
    printf("                                                                          /          /               \n");
    printf("                                                                         /__________/          ______\n");
    printf("                                                                              \n");
    print_planet(planet_terrain);
    if (got_alien != -1) printf("\n\n\n YOU GOT A NEW  ALIEN\n");
    hold_seconds(durtion);
    reset_console();
    printf("                                                                                    ____\n");
    printf("                                                                                   /==\\_\n");
    printf("                                                                                  |         \n");
    printf("                                                                         ---------|    ___     _\n");
    printf("                                                     0    0             \\        |   |___|   |___|   |___|\n");
    printf("                                        0    0      0  0  0  0           /         \\__/ooo\\___/ooo\\___/ooo\\n");
    printf("                                                         0    0         |________     |===|   |===|   \n");
    printf("                                                                                \\_____________________      \n");
    printf("                                                                                          /          /\n");
    printf("                                                                                         /________\n");
    printf("                                                                                              \n");
    print_planet(planet_terrain);
    if (got_alien != -1) printf("\n\n\n YOU GOT A NEW  ALIEN\n");
    hold_seconds(durtion);
    reset_console();
    printf("                                                                                                       \n");
    printf("                                                                                                        \n");
    printf("                                                                                                        \n");
    printf("                                                              0                                   ---- \n");
    printf("                                           0                                                     \\     \n");
    printf("                                                                 0              0                 /   \n");
    printf("                                                                                                 |__ \n");
    printf("                                                                        0                             \n");
    printf("                                                                                                      \n");
    printf("                                                                                               \n");
    printf("                                                                                              \n");
    print_planet(planet_terrain);
    if (got_alien != -1) printf("\n\n\n YOU GOT A NEW  ALIEN\n");
    hold_seconds(durtion);
    reset_console();
    printf("\n\n\n\n\n\n\n\n\n\n\n");
    print_planet(planet_terrain);
	hold_seconds(1);
}

void print_ship_arrive_planet(char* planet_terrain)
{
	double durtion = 0.2;
    hold_seconds(durtion);
    reset_console();
    printf("   ___        \\\n");
    printf("  |___|         >____________\n");
    printf("_/ooo\\_________/            \\\n");
    printf("  |===|                       |\n");
    printf("             __________________/\n");
    printf("              /          \n");
    printf("____________/          \n");
    printf("  \n");
    print_planet(planet_terrain);
    hold_seconds(durtion);
    reset_console();
    printf("___     ___     ___        \\\n");
    printf("___|   |___|   |___|         >____________\n");
    printf("oo\\___/ooo\\___/ooo\\_________/            \\\n");
    printf("===|   |===|   |===|                       |\n");
    printf("                          __________________/\n");
    printf("                          /          \n");
    printf(" _______________________/          \n");
    printf("____/          \n");
    print_planet(planet_terrain);
    hold_seconds(durtion);
    reset_console();
    printf("___________                                \n");
    printf("___________\\                                \n");
    printf("             >_________________________________\n");
    printf("    ___     ___     ___     ___     ___        \\\n");
    printf("   |___|   |___|   |___|   |___|   |___|         >____________\n");
    printf("___/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\_________/            \\\n");
    printf("   |===|   |===|   |===|   |===|   |===|                       |\n");
    printf("___________                                   __________________/\n");
    printf("/          /                                  /          \n");
    printf("__________/          _______________________/          \n");
    printf("         /___________/          \n");
    print_planet(planet_terrain);
    hold_seconds(durtion);
    reset_console();
    printf("     __________________                                \n");
    printf("    /==\\_______________\\                                \n");
    printf("   |                     >_________________________________\n");
    printf("---|    ___     ___     ___     ___     ___     ___        \\\n");
    printf("   |   |___|   |___|   |___|   |___|   |___|   |___|         >____________\n");
    printf("    \\__/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\___/ooo\\_________/            \\\n");
    printf("__     |===|   |===|   |===|   |===|   |===|   |===|                       |\n");
    printf(" \\_____________________                                   __________________/\n");
    printf("           /          /                                  /          \n");
    printf("          /__________/          _______________________/          \n");
    printf("                    /___________/          \n");
    print_planet(planet_terrain);
    hold_seconds(durtion);
    reset_console();
    print_ship();
    print_planet(planet_terrain);
	hold_seconds(1);
}

void print_planet_menu(char* planet_terrain)
{
	print_ship();
	print_planet(planet_terrain);
	printf("                                                           %s planet\n\n", planet_terrain);
}

void print_space_window()
{
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .     .      |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
}

void print_space_window_star_fal()
{
	double durtion = 0.009;
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .     .      |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
	hold_seconds(0.7);
	reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /    / /     \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /   (*)    *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .     .      |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /    / /     \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       / / /      *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     | (*)    .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .     .      |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /    //      \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       / //       *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .     .      |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /   /        \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       ///        *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *    (*|     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .     .      |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       //         *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *   / /|     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .     (*)    |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *  / / |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .    / /     |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .  (*)*     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     * //   |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .   / /      |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      . / / *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .(*) *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *                *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     */     |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .  //        |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      ./ /  *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     / /  *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *          (*)   *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     . /          |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      //    *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .    / /   *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *         / /    *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .            |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *     /      *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .   //     *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *        / /     *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .            |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     . //  .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *       //       *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /          . \\         /                                            \\         /            \\\n");
    printf("   /   *          \\       /           *         *             .     *    \\       /          *   \\\n");
    printf("  |   .     *      |     |    *       .     .        .    *     .   .     |     |  *     .       |\n");
    printf("  |  *   .     .   |     |       .       *       *     .        *      .  |     |    .    *      |\n");
    printf("  |   .    *   .   |     |   .      .        .     .      *  .     *      |     |  .  *     .    |\n");
    printf("  |    *   .    *  |     |        *    .     .    *    .     .            |     |     *    .     |\n");
    printf("  |  .     . *     |     |  .   *     *    .   .    *      .     *     .  |     |   .     *      |\n");
    printf("  |    .    *      |     |     .      .     *     .     .    *     .      |     | .   *     .    |\n");
    printf("   \\    .      *  /       \\       .    *       /        *                /       \\         .    /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
    print_control_panel();
    hold_seconds(durtion);
    reset_console();
    print_space_window();
    print_control_panel();
	hold_seconds(0.7);
	reset_console();
}

void print_space_window_anim()
{
    printf("     ____________           ____________________________________________           ____________\n");
    printf("    /         .  \\         /                                            \\         /            \\\n");
    printf("   /  *           \\       /          *         *             .     *     \\       /         *    \\\n");
    printf("  |  .     *       |     |   *       .     .        .    *     .   .      |     | *     .        |\n");
    printf("  | *   .     .    |     |      .       *       *     .        *      .   |     |   .    *       |\n");
    printf("  |  .    *   .    |     |  .      .        .     .      *  .     *       |     | .  *     .     |\n");
    printf("  |   *   .    *   |     |       *    .     .    *    .     .     .       |     |    *    .      |\n");
    printf("  | .     . *      |     | .   *     *    .   .    *      .     *     .   |     |  .     *       |\n");
    printf("  |   .    *       |     |    .      .     *     .     .    *     .       |     |.   *     .     |\n");
    printf("   \\   .      *   /       \\      .    *                *                 /       \\        .     /\n");
    printf("    \\____________/         \\____________________________________________/         \\____________/\n");
}

void print_big_window() 
{
    if (rand() % 100 < 50)
    {
        printf("     ______________________________________________________________________________________\n");
        printf("    /                                                                                      \\\n");
        printf("   /        *       .        *        .         *        .     *        .         *         \\\n");
        printf("  |   *        .        *       .       *     .      .       *     .       .     *       .   |\n");
        printf("  |        .        *       .       *     .       *       .       *     .       *     .      |\n");
        printf("  |    .     *       .      .   *       .     .      *      .      *      .   *       .      |\n");
        printf("  |       *     .     *        .     *       .    *     .      .    *       .      *     .   |\n");
        printf("  |   .        *     .     *      .      .      *    .       *    .     *      .     .       |\n");
        printf("  |      *     .     .     *       .     .      *      .     *     .     .     *     .       |\n");
        printf("   \\                                                                                        /\n");
        printf("    \\______________________________________________________________________________________/\n");
    }
    else
    {
        printf("     ______________________________________________________________________________________\n");
        printf("    /                                                                                      \\\n");
        printf("   /        .      *         *        .         .        .     *        .         *         \\\n");
        printf("  |   *        .        *       .       *     .      *      *     .       .      *       .   |\n");
        printf("  |        *        *      .       *     .       .       .        *     .       .     *      |\n");
        printf("  |    .     *       .      .   *       .     .      *      .      .      .   *       .      |\n");
        printf("  |       *     .     *        .     *      *    .     .      .    *       *       *     *   |\n");
        printf("  |   *        *     .     .      .      .      .    .       .    *     .      .     .       |\n");
        printf("  |      .     .     *     *       .     .       *      .     *     .     *     .     *      |\n");
        printf("   \\                                                                                        /\n");
        printf("    \\______________________________________________________________________________________/\n");
    }
}
void print_small_window()
{
    if (rand() % 100 < 50)
    {
        printf("      ______________________________________________________________________________________\n");
        printf("     /             *       .        *        .         *        .     *                     \\\n");
        printf("    /        *       .        *        .         *        .     *        .             *     \\\n");
        printf("   |   *        .        *       .          .       .     *         .      .       *      .   |\n");
        printf("   |      *     .     .     *       .     .      *      .     *     .     .     *     .       |\n");
        printf("    \\  .         *     .     *      .      .      *    .       *    .     *                  /\n");
        printf("     \\______________________________________________________________________________________/\n");
    }
    else
    {
        printf("      ______________________________________________________________________________________\n");
        printf("     /          *       .        *        .          *        .     *                       \\\n");
        printf("    /    .           .       *        *        *        .     *         .             .      \\\n");
        printf("   |   *      *          *       .         .       .     *         .       .       *      .   |\n");
        printf("   |      *     .     .     *       .     .       *      .     *     .     .     *    .       |\n");
        printf("    \\  .         *     .     *       *      .      *    .       .    .     *                 /\n");
        printf("     \\______________________________________________________________________________________/\n");
    }


}

void print_control_panel()
{
        printf("                      ___ _                                                    ___                 \n");
        printf("      _______________________________________________________________________________________ \n");
        printf("     | [==     ] [====   ] [========] [==     ] [======  ]  [DOOR] [AIR] [CORE] [ENG] [SHLD] | \n");
        printf("     | ||| |||   ||||     ||||||||   ||| |||   |||||                        ^   ^     ^      | \n");
        printf("     | |_| |_|   |_|      |_____|    |_| |_|   |___|                       ( ) ( )   ( )     | \n");
        printf("     |                                                                                       | \n");
        printf("     |                                                                                       | \n");
        printf("     |    Key binding  :   [^] Up    [v] Down    [ESC] Back / Exit    [ Enter ] Confirm      | \n");
        printf("     |_______________________________________________________________________________________| \n");
        printf("\n");
}

void print_screen_panel()
{
    printf("                     ___ _                                               ___                \n");
    printf("       __________________________________________________________________________________ \n");
    printf("      |  (o) LOG   (o) MAP   (o) SCN   (o) NAV   (o) COM     SYS: Online    TEMP: 243K   |\n");
    printf("      |  +-----+   +-----+   +-----+   +-----+   +-----+      PWR: Stable    O2:   98%%   |\n");
    printf("      |  | LOG |   | MAP |   | SCN |   | NAV |   | COM |      SHD: 72%%      ENG: Nominal |\n");
    printf("      |  +-----+   +-----+   +-----+   +-----+   +-----+                                 |\n");
    printf("      |                                                                                  | \n");
    printf("      |  Key binding :   [^] Up    [v] Down    [ESC] Back / Exit    [ Enter ] Confirm    | \n");
    printf("      |__________________________________________________________________________________| \n");
}

void print_captain_seat()
{
	static int anim = 1;
    if (anim == 1)
    {
		print_space_window_star_fal();
    }

    if (rand() % 100 < 50)
    {
        print_space_window_anim();
        print_control_panel(); 
		anim = 0;
    }    
    else
    {
        print_space_window();
        print_control_panel();
        anim = 0;
    }
}

void print_ship_log(int console_style)
{
    print_big_window();
    print_screen_panel();
    printf("\n");
    if (console_style == 5) // reset ship log 
    {
        printf("\n                          Are you sure you want to reset the ship log?\n\n\n\n");
    }
}

void stars()
{
    if (rand() % 10 < 3)
    {
        printf(" *       .                 *        *        *        .      *        *   .  *     .     \n");
        printf("   .               .              .    *              *   .  *          .     *    *   .  *      *   .  *\n");
        printf("       *              *              .   *                     .  *      .       *  *       .   *\n");
        printf("      *   .              .                      *     .  *                        .      *       \n");
        printf("         .  *       .               *    .            *       .         *             *     .       * \n");
        printf("       *       .                  .                *    .            *    .      *   .        *  \n");
        printf("         .      *   .        *        .   *            .    *        *        .   *     .    *\n");
        printf("          *       .                 .         *          *            .             .  *     .     \n");
        printf("     *   .      *                      *            *       .                  *         .        *  \n");
        printf("       *        .               .  *        *   .             .      *        *   .  *     .     \n");
        printf("         *         .                      .           .           *   .        *     .           *  \n");
        printf("     .      *         *        .   *          .  *    .      *   .        *                . \n");
        printf("\n\n");

	}
	else
	{
        printf(" *       *                 *        *        *        .      *        *   .  *     .     \n");
        printf("   .               .              .    *              *   .  *          .     *    *   .  *      *   .  *\n");
        printf("       *              *              .   *                     .  *      *       *  *       .   *\n");
        printf("      *   .              .                      *     .  *                        .      .       \n");
        printf("         .  *       .               *    *            *       .         *             *     .       * \n");
        printf("       *       .                  .                .    .            *    .      *   .        *  \n");
        printf("         .      *   .        *        .   *            .    *        *        .   *     .    *\n");
        printf("          *       .                 .         *          .            .             .  *     .     \n");
        printf("     *   .      *                      *            *       .                  *         .        *  \n");
        printf("       *        *               .  *        *   .             .      *        *   .  *     *     \n");
        printf("         *         .                      .           .           *   .        *     .           *  \n");
        printf("     .      *         *        .   *          .  .    .      *   .        *                . \n");
        printf("\n\n");
    }
		
}
	